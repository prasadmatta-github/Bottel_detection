...
class svmClassifier:
    def __init__(self, path):
        ...
        self.classifier = svm.SVC(kernel='sigmoid')
    ...
    def split(self, dataset, test_size=0.20):
        # train test split
    ...
    def train(self, test_size=0.20, limit=None):
        # dataset preparation
        print(self.dataset_root,limit)
        dataset = self.load(self.dataset_root, limit)
        (X_train, Y_train), (self.X_test, self.Y_test) = self.split(dataset, test_size)

        # training the model
        print(len(X_train), len(Y_train),type(X_train[0]))
    ...
    def predict(self, image=None, images=None, model_path=None):
        ...
        return [str(x) for x in self.classifier.predict(images)]
...
if __name__ == "__main__":
    ...
    classifier.train()
    for root,dir,files in os.walk('/home/prasadmatta/Documents/bottle/check'):
        for file_ in files:
            path = os.path.join(root,file_)
            img = cv2.imread(path)
            cv2.imshow('{0}'.format(classifier.predict(img)),img)
            cv2.waitKey(0)
